...
fn main() {
    // let (raw_matrix,size): (Vec<isize>,usize) = (vec![
    //     2, 1, 2, -2,
    //     4, 7, 3, -2,
    //     0, 5, -4, 3,
    //     -2, 14, 1, 8,
    // ],4);
    let (raw_matrix, size): (Vec<isize>, usize) = (vec![1, 3, 5, 2, 4, 7, 1, 1, 0], 3);
    let matrix_ring_f64 = MatrixRing::new(F64, size);
    let field = abstalg::ReducedFractions::new(abstalg::I32);
    let matrix_ring = MatrixRing::new(field.clone(), size);

    let mut matrix: Vec<_> = raw_matrix
        .clone()
        .into_iter()
        .map(|i| field.int(i))
        .collect();
    let matrix2: Vec<_> = raw_matrix
        .into_iter()
        .map(|i| abstalg::F64.int(i))
        .collect();

    {
        ...
        println!(
            "P={} L={} U={}",
            Matrix2D::new(field.clone(), &mut p_matrix, size, size),
            Matrix2D::new(field.clone(), &mut l_matrix, size, size),
            Matrix2D::new(field.clone(), &mut u_matrix, size, size),
        );
    }
}

// Now you can use the created matrix for various operations
// For example:
//println!("Determinant of the matrix: {:?}", matrix_ring.determinant(&matrix));
//println!("Determinant of the matrix: {:?}", matrix_ring_f64.determinant(&matrix2));
...
mod tests {
    ...
    #[test]
    fn test_add() {
        let f = ReducedFractions::new(I32);
        let result = f.add(&f.int(2), &f.int(2));
        assert_eq!(result, f.int(4));
    }

    #[test]
    fn test_mul() {
        let f = ReducedFractions::new(I32);
        let result = f.mul(&f.int(2), &f.int(2));
        assert_eq!(result, f.int(4));
    }
    ...
    fn test_div() {
        let f = ReducedFractions::new(I32);
        let result = f.div(&f.int(2), &f.int(2));
        assert_eq!(result, f.int(1));
    }

    #[test]
    fn test_power() {
        let f = ReducedFractions::new(I32);
        let result = f.power(2, &f.int(2));
        assert_eq!(result, f.int(4));
    }
}
